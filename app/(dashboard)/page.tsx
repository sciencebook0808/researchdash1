...
async function getDashboardData() {
  ...
  try {
    ...
    const totalTasks = roadmapSteps.reduce<number>((s, step) => s + step.tasks.length, 0)
    const completedTasks = roadmapSteps.reduce<number>((s, step) => s + step.tasks.filter((t) => t.completed).length, 0)
    ...
    const totalSamples = datasets.reduce<number>((s, d) => s + (d.numSamples || 0), 0)
